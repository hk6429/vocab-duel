...
const PETS = {
  pets: [
    { id: 't1', name: '測靈', ico: '🐱', kind: 's', theme: 'x', skills: ['a', 'b', 'c'], affixes: [{ k: 's', f: '-test' }] },
    { id: 't2', name: '貳靈', ico: '🐶', kind: 's', theme: 'y', skills: ['a', 'b', 'c'], affixes: [{ k: 's', f: '-two' }] }
  ],
  wild: [], skills: { a: {}, b: {}, c: {} }
};
const AFFIXES = {
  prefixes: [], roots: [],
  suffixes: [
    { form: '-test', meaning: 'm', members: ['alpha', 'beta', 'gamma', 'delta', 'omega'] },
    { form: '-two', meaning: 'n', members: ['uno', 'dos'] }
  ]
};
...
test('P3-11 融合鬆綁：Lv15 即可融合、雙親「不降級」、代價僅字幣', async () => {
  const petSave = { owned: { t1: { lv: 15, equip: {} }, t2: { lv: 18, equip: {} } }, active: 't1' };
  const { VDPets, VDGame } = await loadPets({ petSave, coins: 1000 });
  assert.deepEqual(VDPets.canFuse().sort(), ['t1', 't2'], 'Lv15 就該可融合');
  const r = VDPets.fuse('t1', 't2', '幼幼', ['a', 'b', 'c']);
  assert.equal(r.ok, true, `融合應成功：${r.msg || ''}`);
  // 雙親等級不變（舊制會降回 15）
  assert.equal(VDPets.lvOf('t1'), 15);
  assert.equal(VDPets.lvOf('t2'), 18, '雙親等級不應被降級');
  assert.equal(VDGame.raw.coins, 1000 - VDPets.FUSE_COST, '只扣字幣');
  // 幼靈家族＝雙親字綴聯集（5 + 2 = 7 字）
  assert.equal(VDPets.wordsOf(r.id).size, 7);
});

test('P3-11 融合門檻：Lv14 擋下', async () => {
  const petSave = { owned: { t1: { lv: 14, equip: {} }, t2: { lv: 18, equip: {} } }, active: 't1' };
