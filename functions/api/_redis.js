// Redis-over-D1 shim + Vercel→CF Pages 轉接層
// 目的：讓原本跑在 Vercel + Upstash 的 10 支 api/*.js 幾乎原封不動搬到 D1。
//   - redisFor(db)：回傳一個模擬 Upstash Redis 常用指令的物件（底層是 D1 SQL）
//   - vercelToPages(handler)：把 (req,res) 風格 handler 包成 CF Pages 的 onRequest
//
// 契約重點：
//   * get / hget / hgetall / lrange 一律回「原始字串」，呼叫端本來就用
//     `typeof x === 'string' ? JSON.parse(x) : x` 防禦式解析，故相容。
//   * set / hset / lpush 若傳入物件會自動 JSON.stringify（比照 Upstash 行為）。
//   * TTL 存 exp = epoch ms；讀取時 `exp IS NULL OR exp > now` 惰性過期。

export function redisFor(db) {
  const now = () => Date.now();
...
// 把 Vercel (req,res) 風格 handler 包成 CF Pages Functions 的 onRequest
export function vercelToPages(handler) {
  return async ({ request, env }) => {
    ...
    const req = {
      method: m,
      headers: Object.fromEntries([...request.headers].map(([hk, hv]) => [hk.toLowerCase(), hv])),
    ...
    const outHeaders = {};
    const res = {
      setHeader(hk, hv) { outHeaders[hk] = String(hv); return this; },
      status(c) { status = c; return this; },
      json(o) { return new Response(JSON.stringify(o), { status, headers: { ...outHeaders, 'Content-Type': 'application/json; charset=utf-8' } }); },
      send(o) { return new Response(o, { status, headers: outHeaders }); },
      end() { return new Response(null, { status, headers: outHeaders }); },
    };
